...
1. Click **Save** and wait for the configuration to apply.
...
1. To configure different fields for different document types, create a pipeline for each index with if condition inside it:
...
Set the **ElasticSearch9Options:EnableDebugMode** property to **true** in your application configuration. This will:

* Disable direct streaming of requests/responses (DisableDirectStreaming()).
* Allowing raw JSON payloads to be captured. Format JSON output for improved readability (PrettyJson()).
* Request full error stack traces from the Elasticsearch server on failures.
