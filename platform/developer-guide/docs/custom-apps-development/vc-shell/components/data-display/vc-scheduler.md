...
<!-- Source: ui/components/organisms/vc-scheduler/vc-scheduler.docs.md -->
...
- All three anchored popovers -- "+N more" overflow, quick-info and quick-create -- are `VcPopover` panels carrying `role="dialog"` and an accessible name (the formatted date, the event title, and "New event" respectively). They do **not** trap focus: `VcPopover` has no focus management of its own, so each one moves focus into itself on open and hands it back to whatever opened it on close. Focus is repaired, not dictated -- a user who moves focus out of the panel keeps it there when the panel closes.
- The editor modal and the This event/All events scope dialog are `VcPopup`, which is a Headless UI `Dialog` -- those do get accessible naming, a focus trap, focus restore and `inert` on the background from the library.
...
- [VcDataTable](./vc-data-table.md) -- for tabular (non-calendar) views of the same periods.
- [VcGallery](./vc-gallery.md) -- another organism with drag interaction, for image reordering rather than time-based events.
