...
| `onParentCall`         | `(parentExposedMethods: Record<string, any>, args: IParentCallArgs) => void`                               | (Primarily for internal use by `VcBladeNavigation` infrastructure but shows the pattern). Handles the `parent:call` event emitted by child blades. `parentExposedMethods` would be the `exposed` object of the parent. `args` is `IParentCallArgs { method: string, args?: any, callback?: (result: any) => void }`. |
