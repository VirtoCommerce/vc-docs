...
<!-- Source: core/composables/useKeyboardShortcuts/useKeyboardShortcuts.docs.md -->
...
- **Declaring a shortcut on a toolbar item.** The common case: add `shortcut: hotkey.mod.s` next to `clickHandler` on an `IBladeToolbar` entry (array pattern or `useToolbar().registerToolbarItem`, see [useToolbar](../services/useToolbar.md)).
...
- [useToolbar](../services/useToolbar.md) -- registers the toolbar items that carry `shortcut`; see its `IBladeToolbar`/`IToolbarItem` reference for the full item shape
...
- `IBladeToolbar` in [Core types](../../reference/api/types-core.md) -- the `shortcut?: ShortcutDefinition` field lives here
