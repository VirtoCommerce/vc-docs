...
> **Deprecated.** `useAssets` is deprecated in favor of [`useAssetsManager(ref, options)`](./useAssetsManager.md). Calling it logs a DEV console warning. This page is kept as legacy/migration guidance; new code should use `useAssetsManager`, which wraps `useAssets` with two-way sync.
