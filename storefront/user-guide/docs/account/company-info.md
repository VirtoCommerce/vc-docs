# Company Info

The **Company info** section contains the following widgets:

* **Company name** to edit and save company name:

    ![Company name widget](../media/company-name-widget.png){: style="display: block; margin: 0 auto;" }

* **Company logo** (for admins) to upload, edit, or delete company logo in JPG and PNG format not exceeding 4.8 MB. Deleting the newly uploaded logo restores the default logo:
    <br>
    <br>
    ![Upload company logo](../media/upload-company-logo.gif)
    <br>
    <br>

* **Addresses** to add, delete, or add company addresses to favorites, including billing and delivery addresses:

    ![Addresses widget](../media/addresses-widget.png){: style="display: block; margin: 0 auto;" }

    !!! info
        The favorite address will automatically appear at the top of the address list in the **Company Info** section and during checkout.


<br>
<br>
********

<div style="display: flex; justify-content: space-between;">
    <a href="../points-history">← Points history</a>
    <a href="../company-members">Company members →</a>
</div>