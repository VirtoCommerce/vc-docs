...
This tutorial provides an overview of the steps involved in creating a reverse proxy using Azure Application Gateway to make your single-page application SEO-friendly by using Prerender Seamlessly Renders JavaScript for Faster Indexing for efficient rendering. Following these steps, we will deploy a public proxy, accessible by the https://vc-prerender.westeurope.cloudapp.azure.com URL, that will forward requests to virtostart-demo-store.govirto.com using the prerendering capabilities provided by the Prerender.io service.
...
1. Configure **Backends**:
    ...
    1. Add a pool for our downstream frontend https://virtostart-demo-store.govirto.com which runs as a single-page application written in Vue.js. This pool will be used to forward all requests to static theme resources directly to downstream service with bypassing prerender.io. Fill in the fields as follows, then click **Add**:
...
1. Manage **Configuration** to connect the frontend and backend pool you created using a routing rule:
    1. Click **Add a routing rule** in the **Routing rules** column. A single routing rule (path-based) that will forward by default all incoming requests to the service.prerender.io by default and all requests to the static resources with the paths matched to this pattern  `/static/*`,`/themes/*` directly to virtostart-demo-store.govirto.com.
    1. In the new window, fill in the fields in the **Listener** tab. Fill in the fields as follows and switch to the **Backend targets** tab:
        ...
        Specify the existing SSL certificate, which was issued for your public domain. In our case, we use RSA certificate (.pfx) issued by the ZeroSSL service for the domain **vc-prerender.westeurope.cloudapp.azure.com**.
    ...
    1. The path appears in the paths section, and the rule looks as follows:
...
1. Configure **UrlRewrite** rules to redirect all incoming **GET** requests from public IP to **service.prerender.io**. 
...
After applying changes, check the result. To simulate crawler requests, use Postman, set the User-Agent header to googlebot, and check that the result contains the pre-rendered version of the site.
